...
static char            *D_800BD2D0;
static int             D_800BD2D4;
...
/* global workspace */
unsigned long __musvelocity;
unsigned long __muspad0[2];
unsigned long __musvolume;
unsigned long __muspad1[2];
unsigned long __musenv_current;
unsigned long __muspad2[2];
unsigned long __muscontrol_flag;                /* music player control flag */
unsigned long __muspad3[2];
unsigned long __musvolscale;
unsigned long __muspad4[2];
unsigned long __musenv_count;
...
static ALMicroTime __MusIntMain(void *node)
{
  ...
  for(x=-MAX_SONGS, cp=mus_channels; x<max_channels-MAX_SONGS; x++, cp++)
  {
    ...
    if(cp->length != 0x7fff)
    {
      while (((long)(cp->note_end_frame<cp->channel_frame)) && cp->pdata!=NULL)
    ...
    if(cp->pvolume && ((long)(cp->volume_frame<cp->channel_frame)))
      __MusIntProcessContinuousVolume(cp);
    if(cp->ppitchbend && ((long)(cp->pitchbend_frame<cp->channel_frame)))
    ...
    if(cp->playing)
    {
      ...
      if(cp->sweep_speed && ((long)(cp->sweep_frame<cp->channel_frame)))
...
{
  u32	volume;
  __musvolume = cp->volume;
  __musenv_current = cp->env_current;
  __musvelocity = cp->velocity;
  __musvolscale = cp->volscale;

  /* process volume */
  volume = ((u32)(cp->volume)*(u32)(cp->env_current)*(u32)(cp->velocity)*(u32)(cp->volscale))>>13;
  if (cp->song_addr)
  {
    volume = (volume * D_800BD2D0[cp->song_addr->wave_table[cp->wave]]) >> 7;
  }
  if (volume>32767)
    volume = 32767;
