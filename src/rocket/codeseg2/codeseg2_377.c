...
// Minor regalloc and function argument load order
#ifdef NON_MATCHING
void func_800882B8(struct unkfunc_800882B8 *arg0, u32 romAddr, s32 arg2) {
    ...
    if (arg2 != 0) {
        ...
        sp10.dataPtr += toSkip * sizeof(s32);
    }
    ...
    while (1) {
        ...
        sp20.dataPtr = alloc_second_heap(dataLen2);
        dma_read(romAddr, sp20.dataPtr, dataLen2);
        romAddr += dataLen2;
        temp_s1 = D_800AC7F0[READ_VALUE(sp20.dataPtr, s32)];
        temp_s2 = READ_VALUE(sp20.dataPtr, s32);
        ...
        curModel->unk0->unk20(curModel, &romAddr, sp20.dataPtr); // func_8001E044 potentially
        phi_s3++;
    }
    D_800ADAE4 = s7;
}
